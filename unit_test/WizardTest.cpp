...
//Wizard(float x, float y, float str = DEF_HERO_STRENGTH, bool onf = false, float m = 10., float h = DEF_HERO_HEALTH, bool facingR = true, float s = HERO_SPEED);
...
TEST(WizardTest, Constructor) {
    ...
    EXPECT_EQ(DEF_HERO_STRENGTH, w.getStrength());
    EXPECT_EQ(false, w.isOnFire());
    EXPECT_EQ(DEF_HERO_HEALTH, w.getHealth());
...
TEST(WizardTest, TrueOnFire) {
    Wizard w(1,1, DEF_HERO_STRENGTH, true);
    EXPECT_EQ(1, w.getSprite().getPosition().x);
    EXPECT_EQ(1, w.getSprite().getPosition().y);
    EXPECT_EQ(DEF_HERO_STRENGTH, w.getStrength());
...
TEST(WizardTest, NotDefHealth) {
    Wizard w(1,1, DEF_HERO_STRENGTH, false, 10., 34.);
    EXPECT_EQ(1, w.getSprite().getPosition().x);
    EXPECT_EQ(1, w.getSprite().getPosition().x);
    EXPECT_EQ(DEF_HERO_STRENGTH, w.getStrength());
    EXPECT_EQ(false, w.isOnFire());
...
TEST(WizardTest, NotDefMana) {
    Wizard w(1,1, DEF_HERO_STRENGTH, false, 3., DEF_HERO_HEALTH);
    EXPECT_EQ(1, w.getSprite().getPosition().x);
    EXPECT_EQ(1, w.getSprite().getPosition().x);
    EXPECT_EQ(DEF_HERO_STRENGTH, w.getStrength());
    EXPECT_EQ(false, w.isOnFire());
...
TEST(WizardTest, IsFacingLeft) {
    Wizard w(0, -5, DEF_HERO_STRENGTH, false, 10., DEF_HERO_HEALTH, false);
...
TEST(WizardTest, MovementSpeed) {
    Wizard w(1, -1, DEF_HERO_STRENGTH, false, 10., DEF_HERO_HEALTH, true, 35.);
    EXPECT_EQ(1, w.getSprite().getPosition().x);
    EXPECT_EQ(-1, w.getSprite().getPosition().y);
    EXPECT_EQ(DEF_HERO_STRENGTH, w.getStrength());
    EXPECT_EQ(false, w.isOnFire());
...
TEST(WizardTest, Assigment) {
    Wizard w(1, 1, DEF_HERO_STRENGTH, true, 10., 23., true, -90.);
    Wizard newW = w;
...
TEST(WizardTest, Copy) {
    Wizard w(-1, -1, DEF_HERO_STRENGTH, false, 8., 10., false, 2.);
    Wizard copy(w);
